...
class EyettentionBaseline(nn.Module):
	def __init__(self, cf):
		super(EyettentionBaseline, self).__init__()
		self.cf = cf
	...
	def decode(self, sp_emd, sn_mask, sp_pos, enc_out, sp_fix_dur, sp_landing_pos, word_ids_sp):
		...
		# Fixation-Sequence Encoder + Decoder
    	# Initialize hidden state and cell state with zeros,
		hn = torch.zeros(8, enc_out.shape[0], self.hidden_size).to(enc_out.device)
		hc = torch.zeros(8, enc_out.shape[0], self.hidden_size).to(enc_out.device)

		# ... [Initialize hx, cx, hx2, cx2, ..., hx8, cx8 as before] ...

		# Predict output for each time step in turn
		output = []
		# Save attention scores for visualization
		atten_weights_batch = torch.empty(enc_out.shape[0], 0, self.cf["max_sn_len"]).to(enc_out.device)
		...
		for i in range(enc_out.shape[0]):	
			hx, cx = self.decoder_cell1(enc_out[i], (hx, cx))     # [batch, units]
			...
			att_weight = self.cross_attention(ht=hx8,
										hs=enc_out,
										sn_mask=sn_mask,
										cur_word_index=None)  # TODO Adjust 'cur_word_index' as necessary
